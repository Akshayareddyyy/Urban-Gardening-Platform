...
export function LoginForm() {
  ...
  const handleSubmit = async (values: LoginFormValues) => {
    ...
    try {
      ...
      console.error("Error signing in:", error); // Logs the detailed FirebaseError to the console
      ...
      if (error.code) {
        switch (error.code) {
          ...
          case 'auth/invalid-credential': // This handles the error you're seeing
            errorMessage = "Invalid email or password. Please check your credentials and try again.";
            break;
          case 'auth/api-key-not-valid':
            errorMessage = "Firebase configuration error (API key). Please contact support.";
            break;
          case 'auth/network-request-failed':
            errorMessage = "Network error. Please check your internet connection and try again.";
            break;
          default:
            // Use Firebase's error message if it's somewhat user-friendly, otherwise a generic one.
            errorMessage = error.message || "An unexpected error occurred. Please try again.";
            // Avoid overly technical default Firebase messages if possible
            if (error.message && (error.message.includes("INTERNAL ASSERTION FAILED") || error.message.length > 150)) {
